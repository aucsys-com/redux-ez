...
export function makeAsyncListSliceActions({
  ...
  const asyncThunk = createAsyncThunk(thunkName, async (arg, thunkAPI) => {
  ...
  const extraReducers = {
    [asyncThunk.pending]: (state, action) => {
      if (!state[stateNames.loading][action.meta.arg.id]) {
        ...
        state[stateNames.currentRequestId][action.meta.arg.id] =
    ...
    [asyncThunk.fulfilled]: (state, action) => {
      ...
      if (
        state[stateNames.loaderIndicator][action.meta.arg.id] &&
        state[stateNames.currentRequestId][action.meta.arg.id] === requestId
      ) {
        if (onFulfilledFunc) {
    ...
    [asyncThunk.rejected]: (state, action) => {
      const { requestId } = action.meta
      if (
        state[stateNames.loading][action.meta.arg.id] &&
        state[stateNames.currentRequestId][action.meta.arg.id] === requestId
      ) {
        state[stateNames.loading][action.meta.arg.id] = false
        state[stateNames.error][action.meta.arg.id] =
