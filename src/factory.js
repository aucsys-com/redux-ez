...
const ReduxEz = ({ baseUrl, makeHeaders }) => {
  ...
  const getSliceActions = (settings) => {
    return makeAsyncSliceActions({
      ...
      thunkAction: async (req, thunkAPI) =>
        await client.get(settings.makePath(req) || '')
    })
  }
  ...
  const createSliceActions = (settings) => {
    return makeAsyncSliceActions({
      ...
      onFulfilledFunc: setEntityOnFulfilled,
      thunkAction: async (req, thunkAPI) =>
        await client.post(settings.makePath(req) || '', req.body)
    })
  }
  ...
  const updateSliceActions = (settings) => {
    return makeAsyncSliceActions({
      ...
      onFulfilledFunc: setEntityOnFulfilled,
      thunkAction: async (req, thunkAPI) =>
        await client.patch(settings.makePath(req) || '', req.body)
    })
  }
  ...
  const deleteSliceActions = (settings) => {
    return makeAsyncSliceActions({
      ...settings,
      thunkAction: async (req, thunkAPI) =>
        await client.delete(settings.makePath(req) || '')
    })
  }
